...
export default function RenterTable({ renters, onDelete, onEdit }: RenterTableProps) {
  const columns: Column<Renters>[] = [
    {
      header: "Renter",
      render: (renter) => (
    ...
    },
    {
      header: "Phone",
      render: (renter) => (
        <span className="text-sm text-gray-600 dark:text-gray-300">{renter.phoneNumber || "—"}</span>
      ),
    },
    {
      header: "NIDA No",
      render: (renter) => (
        <span className="text-sm text-gray-600 dark:text-gray-300">{renter.nidaNumber || "—"}</span>
      ),
    },
    {
      header: "Status",
      render: (renter) => (
        <Badge size="sm" color={renter.isActive ? "success" : "error"}>
    ...
    },
    {
      ...
      render: (renter) => (
        <ActionDropdown
          onEdit={() => onEdit(renter)}
          onDelete={() => onDelete(renter.uuid)}
        />
      ),
    },
  ];
